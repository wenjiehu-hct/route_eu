...
export default function POIEditor() {
  ...
  const reset = () => { setEditingId(null); setForm(EMPTY); store.setMapPickEnabled(false); };
  useEffect(() => {
    const picked = event => setForm(current => ({ ...current, lat: event.detail.lat.toFixed(6), lon: event.detail.lon.toFixed(6), name: current.name || '地图标记点' }));
    window.addEventListener('poi-map-picked', picked);
    return () => window.removeEventListener('poi-map-picked', picked);
  }, []);
  const toggleMapPick = () => {
    useRoutePlannerStore.getState().setMapPickEnabled(false);
    store.setMapPickEnabled(!store.mapPickEnabled);
  };
  const save = () => {
  ...
  </Card><Card title={editingId ? '编辑标记点' : '新增标记点'} actions={<Button size="sm" variant={store.mapPickEnabled ? 'primary' : 'secondary'} onClick={toggleMapPick}>{store.mapPickEnabled ? '请点击地图…' : '从地图选点'}</Button>}>
