...
const { useComplianceStore, normalizeProject, getRunResourceConflicts } = await import('../src/stores/useComplianceStore.js');
...
useComplianceStore.getState().submitTestRun(project.id, testRun.id, { startOdometer: 1000, endOdometer: 1052.4 });
assert(useComplianceStore.getState().projects[0].testRuns[0].status === 'review', 'Completed field work did not enter engineering review');
useComplianceStore.getState().approveTestRun(project.id, testRun.id, { reviewer: 'Review Engineer', reviewNotes: 'Evidence accepted' });
...
assert(expandedProject.testRuns[0].reviewer === 'Review Engineer' && expandedProject.testRuns[0].reviewedAt, 'Engineering review audit fields were not saved');
...
useComplianceStore.getState().updateTestRun(project.id, testRun.id, { notes: 'tampered', status: 'running' });
useComplianceStore.getState().setRunScenarioResult(project.id, testRun.id, 'isa_total_distance', { status: 'failed' });
useComplianceStore.getState().deleteRunCheckpoint(project.id, testRun.id, expandedProject.testRuns[0].checkpoints[0].id);
const frozenRun = useComplianceStore.getState().projects[0].testRuns.find(item => item.id === testRun.id);
assert(frozenRun.status === 'completed' && frozenRun.notes !== 'tampered' && frozenRun.scenarioResults.isa_total_distance.status === 'passed' && frozenRun.checkpoints.length === 1, 'Archived run records were not immutable');
...
const conflictA = useComplianceStore.getState().addTestRun(project.id, { name: 'Conflict A', date: '2026-07-24', plannedStart: '09:00', plannedEnd: '11:00', driver: 'Driver B', vehicle: 'Vehicle B' });
useComplianceStore.getState().addTestRun(project.id, { name: 'Conflict B', date: '2026-07-24', plannedStart: '10:00', plannedEnd: '12:00', driver: 'Driver B', vehicle: 'Vehicle B' });
assert(getRunResourceConflicts({ ...conflictA, projectId: project.id }, useComplianceStore.getState().projects).length === 2, 'Driver and vehicle schedule conflicts were not detected');
...
assert(csv.includes('Munich execution') && csv.includes('ISA sign mismatch') && csv.includes('Review Engineer') && csv.includes('Evidence accepted'), 'Project CSV export omitted execution or review audit data');
