...
app.whenReady().then(async () => {
  try {
    ...
    const interaction = await window.webContents.executeJavaScript(`(async () => {
      ...
      const mapPickButton = [...document.querySelectorAll('.planning-controls button')].find(button => button.textContent.includes('启用选点') || button.textContent.includes('暂停选点'));
      if (mapPickButton?.textContent.includes('启用选点')) mapPickButton.click();
      await new Promise(resolve => setTimeout(resolve, 80));
      const planningMap = document.querySelector('.planning-map-workspace .leaflet-container');
      const mapRect = planningMap?.getBoundingClientRect();
      if (planningMap && mapRect) planningMap.dispatchEvent(new MouseEvent('click', { bubbles: true, clientX: mapRect.left + mapRect.width * .55, clientY: mapRect.top + mapRect.height * .45 }));
      const waypointMapPickWorks = await waitFor('.planning-controls .stop-list article');
      [...document.querySelectorAll('.planning-controls button')].find(button => button.textContent.includes('清空草稿'))?.click();
      document.querySelectorAll('.planning-methods > button')[1]?.click();
      await waitUntil(() => document.querySelector('.planning-controls')?.textContent.includes('绘制测试区域'));
      [...document.querySelectorAll('.planning-controls button')].find(button => button.textContent.includes('绘制测试区域'))?.click();
      const polygonDrawingStarted = await waitUntil(() => document.querySelector('.planning-controls')?.textContent.includes('取消绘制'));
      document.querySelectorAll('.planning-methods > button')[0]?.click();
      await waitUntil(() => document.querySelector('.planning-controls')?.textContent.includes('常规 Waypoint 路线规划'));
      const waypointAutoPickEnabled = [...document.querySelectorAll('.planning-controls button')].some(button => button.textContent.includes('暂停选点'));
      await new Promise(resolve => setTimeout(resolve, 80));
      const manualMap = document.querySelector('.planning-map-workspace .leaflet-container');
      const manualMapRect = manualMap?.getBoundingClientRect();
      if (manualMap && manualMapRect) manualMap.dispatchEvent(new MouseEvent('click', { bubbles: true, clientX: manualMapRect.left + manualMapRect.width * .48, clientY: manualMapRect.top + manualMapRect.height * .52 }));
      const waypointModeIsolationWorks = await waitFor('.planning-controls .stop-list article');
    ...
    if (!interaction.waypointMapPickWorks) errors.push('Waypoint map click did not add a draft stop');
    if (!interaction.polygonDrawingStarted || !interaction.waypointAutoPickEnabled || !interaction.waypointModeIsolationWorks) errors.push('Waypoint and polygon planning modes leaked state or blocked map clicks');
